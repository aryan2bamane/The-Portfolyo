...
const AllServices = () => {
  ...
  return (
    <>
      <div className="md:grid md:grid-cols-3 w-full gap-5 lg:py-10  justify-center items-center md:my-5">
        {userData && userService ? (
          <>
            {userService.map((service) => (
              <div
                className="w-full bg-[#e9e9e9]  lg:h-full flex flex-col justify-center items-center gap-2 p-10 border-2 border-slate-300 my-5 md:h-full"
                key={service._id}
              >
